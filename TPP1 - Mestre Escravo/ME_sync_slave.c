...
main(int argc, char ** argv) {
    ...
    if ( my_rank == 0 ){    // papel do mestre
        ...
        while(1){                               // inicio o laço recebendo por ordem de chegada com any_source
            ...
            if(atual<TAREFAS)   // Se nao ordenei todas ainda entao envia para o escravo que finalizou
                MPI_Send( &saco[atual], TAM, MPI_INT, status.MPI_SOURCE, atual, MPI_COMM_WORLD);
        ...
        for ( i=0 ; i < proc_n-1; i++) // Envia Kill para todos os escravos
            MPI_Send( &saco[0], TAM, MPI_INT, i+1, 99999, MPI_COMM_WORLD); // envio tag kill para os escravos
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        MPI_Send(aux, TAM, MPI_INT, 0, tagSTART, MPI_COMM_WORLD);

        while(1){
            MPI_Recv(aux, TAM, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);    // recebo do mestre
            ...
            MPI_Send(aux, TAM, MPI_INT, 0, status.MPI_TAG, MPI_COMM_WORLD);
        }
    }
